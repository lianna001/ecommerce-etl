...
def run_send_email(**kwargs):
    ...
    send_daily_report(date_str=date_str, insight=insight, news=news)
...
def run_update_notion(**kwargs):
    ...
    insight  = ti.xcom_pull(key="insight", task_ids="generate_insight")
    news     = ti.xcom_pull(key="news",    task_ids="fetch_news")
    update_notion_page(date_str=date_str, insight=insight, news=news)
...
with DAG(
    ...
    t3 = PythonOperator(task_id="fetch_news",              python_callable=run_fetch_news)
    t4 = PythonOperator(task_id="send_email",              python_callable=run_send_email)
    t5 = PythonOperator(task_id="update_notion",           python_callable=run_update_notion)
